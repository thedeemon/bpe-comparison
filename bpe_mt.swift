...
struct PairWalker {
    ...
    init(_ from : Int, _ before : Int) {
        ...
        walker = WalkIter(from, data.count)
        p1 = walker.next()
        p2 = walker.next()
    }
...
func calcPairHisto() -> [VPair : Int] {
    var h : [VPair : Int] = [:]
    var pairs = PairWalker(0, data.count)
...
func findUnchangingSlot(_ pos: Int, _ v1: UInt16, _ v2: UInt16) -> Int {
    if (pos >= data.count) { return data.count }
    var p = pos
    while p < data.count && (data[p] == 0 || data[p] == v1 || data[p] == v2) {
        p += 1
    }
    return p
}

func replacePairInSegment(_ walker: PairWalker, _ oldValuePair : VPair, _ newVal : UInt16, _ deltas: inout [VPair : Int]) {
    var ph = deltas   // use local copies on the stack
    var pairs = walker
...
func replacePair(_ oldValuePair : VPair, _ newVal : UInt16, _ ph: inout [VPair : Int]) {
    var segments : [PairWalker] = []
    var deltas : [[VPair : Int]] = []
    if data.count < 512 {
        segments.append( PairWalker(0, data.count) )
        deltas.append([:])
    } else {
        let segSize = (data.count + 31) / 32
        var st = 0
        var en = 0
        while en < data.count {
            st = en
            en = findUnchangingSlot(en + segSize, oldValuePair.value1, oldValuePair.value2)
            segments.append( PairWalker(st, en) )
            deltas.append([:])
        }
    }
    DispatchQueue.concurrentPerform(iterations: segments.count) { i in
        replacePairInSegment(segments[i], oldValuePair, newVal, &deltas[i] )
    }
    for dlt in deltas {
        for (k,v) in dlt {
            ph[k, default: 0] += v
...
func oneStep(_ step : Int, _ ph: inout [VPair : Int]) -> Bool {
    ...
    if Double(total) < Double(data.count) * 0.707 {
        ...
        var iter = WalkIter(0, data.count)
